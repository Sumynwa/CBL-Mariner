...
# filenames for each possibly-rehydrated RPM. In cases where an RPM is unavailable/download fails, we create an
# empty file. The build_official_toolchain_rpms.sh script knows the distinction between empty/non-empty files.
# When is it meaningful to attempt to partially rehydrate? It makes no sense to partially rehydrate if:
# - INCREMENTAL_TOOLCHAIN = n: We won't use these RPMs- this  rebuild from scratch, then
# - REBUILD_TOOLCAHIN = n: We defer to the download step that errors when a package not 
# - TOOLCHAIN_PARTIAL_REPO_REHYDRATION = n: This flag explicitly disables this functionality
# In these cases, we just create empty files for each possible rehydrated RPM.
ifeq ($(strip $(INCREMENTAL_TOOLCHAIN))$(strip $(REBUILD_TOOLCHAIN))$(strip $(TOOLCHAIN_PARTIAL_REPO_REHYDRATION)),yyy)
